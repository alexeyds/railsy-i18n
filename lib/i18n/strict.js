...
module.exports = class I18nStrict extends I18nBase {
  ...
  _checkMissingInterpolation(interpolation, path) {
    if (interpolation.unusedReplacements) {
      ...
      throw new InterpolationError(`Unused placeholder replacements for '${path}': ${replacements}`);
    } 

    if (interpolation.undefinedReplacements) {
      let replacements = toStr(interpolation.undefinedReplacements);

      throw new InterpolationError(`Undefined placeholder replacements for '${path}': ${replacements}`);
    }
    ...
    if (interpolation.remainingPlaceholders) {
      ...
      throw new InterpolationError(
        `Missing placeholder replacements: expected to receive {${placeholders}} for translation at '${path}'`
      );
    }
  }
};
